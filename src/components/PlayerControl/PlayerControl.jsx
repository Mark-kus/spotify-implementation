...
export default function PlayerControl({ tracks }) {
  ...
  const [trackPlaying, setTrackPlaying] = useState(tracks[0])
  ...
  const nextTrack = useCallback(() => {
    if (currentIndex >= tracks.length - 1 || tracks.length <= 1) return;
    const nextIndex = currentIndex + 1;
    setCurrentIndex(nextIndex);
    if (nextIndex === tracks.length - 1)
      setControlAvailable({ ...controlAvailable, hasNext: false });
    if (!controlAvailable.hasPrevious)
      setControlAvailable({ ...controlAvailable, hasPrevious: true });
  ...
  }, [currentIndex, tracks, controlAvailable, music, isPlaying]);
  ...
  const previousTrack = () => {
    ...
    if (previousIndex === 0)
      setControlAvailable({ ...controlAvailable, hasPrevious: false });
    if (!controlAvailable.hasNext)
      setControlAvailable({ ...controlAvailable, hasNext: true });
  ...
  return (
    <>
      <div className="music-controls">
            <div id="track-info">
                <h3>{trackPlaying.name}</h3>
            </div>
        <MusicBar trackProgress={trackProgress} />

        <button
          className={`${controlAvailable.hasPrevious ? "button" : null}`}
          id="previous-track"
          onClick={previousTrack}
        >
          <svg
            fill={`${controlAvailable.hasPrevious ? "white" : "gray"}`}
            role="img"
            height="12"
            width="12"
            aria-hidden="true"
        ...
        <button
          className={`${controlAvailable.hasNext ? "button" : null}`}
          id="next-track"
          onClick={nextTrack}
        >
          <svg
            fill={`${controlAvailable.hasNext ? "white" : "gray"}`}
            role="img"
            height="12"
            width="12"
            aria-hidden="true"
        ...
        <button
          className={`${controlAvailable.playable ? "button" : null}`}
          onClick={toggleMusicPlay}
        >
          {isPlaying ? (
            <svg
              fill={`${controlAvailable.playable ? "white" : "gray"}`}
              role="img"
              height="24"
              width="24"
              aria-hidden="true"
